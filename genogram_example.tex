\documentclass[svgnames,border=1mm, 11pt]{standalone}
\usepackage{genogram}

\begin{document}
    \brother[
        of=husband, age=35, name=Bruce,
        extra node feature={Navy, fill=Navy!10},
        extra label feature={text=Navy}
    ]
    \sisterInLaw[
        of=husband, age=32, name=Selina,
        extra node feature={Plum, fill=Plum!10},
        extra label feature={text=Plum}
    ]
    \niece[
        of=husband, age=9, name=Robin,
        extra node feature={OliveDrab, fill=OliveDrab!10},
        extra label feature={text=OliveDrab}
    ]
    \husband[
        name=Clark, age=30, status=not married,
        extra node feature={red, fill=yellow!10},
        extra label feature={text=red}
    ]
    \sister[
        of=wife, age=36, name=Lucy,
        extra node feature={magenta, fill=magenta!10},
        extra label feature={text=magenta}
    ]
    \wife[
        age=31, name=Lois,
        extra node feature={violet, fill=violet!10},
        extra label feature={text=violet}
    ]
    \son[
        male, age=7, name=Tony,
        extra node feature={orange, fill=orange!10},
        extra label feature={text=orange}
    ]
    \daughter[
        female, age=5, name=Pepper,
        extra node feature={blue, fill=blue!10},
        extra label feature={text=blue}
    ]
    \mother[
        of=wife, age=65, name=Ella,
        extra node feature={Teal, fill=Teal!10},
        extra label feature={text=Teal}
    ]
    \father[
        of=wife, age=68, name=Sam,
        extra node feature={Goldenrod, fill=Goldenrod!10},
        extra label feature={text=Goldenrod}
    ]
    \mother[
        of=husband, dead, age=25, name=Lara,
        extra node feature={LightSeaGreen, fill=LightSeaGreen!10},
        extra label feature={text=LightSeaGreen}
    ]
    \father[
        of=husband, dead, age=32, name=Thomas, status=divorced,
        extra node feature={FireBrick, fill=FireBrick!10},
        extra label feature={text=FireBrick}
    ]
    \setBeforeDrawing[
        husband parents distance=12em,
        couple distance=20em,
        siblings generation add distance below=-5mm
    ]
    \addToTikzpicture{%
        \begin{scope}[every node/.style={inner sep=0, inner ysep=1mm, font=\ttfamily\tiny}]
            \node[text=red, anchor=north west] at (gen@husband.north east) {
                \begin{tabular}{r@{}l}
                    \char`\\husband[ & name=Clark,\\
                    & age=30, \\
                    & status=not married]
                \end{tabular}
            };
            \node[text=violet, anchor=west] at (gen@wife.east) {
                \begin{tabular}{r@{}l}
                    \char`\\wife[ & age=31,\\
                    &name=Lois]
                \end{tabular}
            };
            \node[text=orange, left = 1mm of gen@child1] {\char`\\child[male, age=3, name=Tony]};
            \node[text=blue,    right = 1mm of gen@child2] {\char`\\child[female, age=5, name=Pepper]};
            \node[text=magenta, below = 0mm of gen@wife@sibling1] {
                \begin{tabular}{r@{}l}
                    \char`\\sister[ & of=wife,\\
                    &age=36,\\
                    &name=Lucy]
                \end{tabular}
            };
            \node[text=Navy, anchor=east] at (gen@husband@sibling1.south west) {
                \begin{tabular}{r@{}l}
                    \char`\\brother[ & of=husband,\\
                    &age=35,\\
                    &name=Bruce]
                \end{tabular}
            };
            \node[text=Plum, anchor=south east] at (gen@husband@sibling1.north) {
                \begin{tabular}{r@{}l}
                    \char`\\sisterInLaw[ & of=husband,\\
                    & age=32,\\
                    & name=Selina]
                \end{tabular}
            };
            \node[text=OliveDrab, anchor=east] at (gen@husband@sibling1@child1.west) {
                \begin{tabular}{r@{}l}
                    \char`\\niece[ & of=husband,\\
                    & age=9, name=Robin]
                \end{tabular}
            };
            \node[text=FireBrick, anchor=north west] at (gen@husband@father.north east) {
                \begin{tabular}{r@{}l}
                    \char`\\father[ & of=husband,\\
                    & dead, age=32, \\
                    & name=Thomas, \\
                    & status=divorced]
                \end{tabular}
            };
            \node[text=LightSeaGreen, anchor=north west] at (gen@husband@mother.north east) {
                \begin{tabular}{r@{}l}
                    \char`\\mother[ & of=husband,\\
                    & dead, age=25, \\
                    & name=Lara
                \end{tabular}
            };
            \node[text=Goldenrod, anchor=north west] at ($(gen@wife@father.east)!0.5!(gen@wife@father.south east)$) {
                \begin{tabular}{r@{}l}
                    \char`\\father[ & of=wife,\\
                    & age=68, \\
                    & name=Sam]
                \end{tabular}
            };
            \node[text=Teal, anchor=east] at (gen@wife@mother.north west) {
                \begin{tabular}{r@{}l}
                    \char`\\mother[ & of=wife,\\
                    & age=65, \\
                    & name=Ella]
                \end{tabular}
            };
        \end{scope}
    }
    \drawGenogram
\end{document}
